 ...
 * optimize for serialized memory size, which is why we pack all the strings together into a single string before and
 * serialize that, instead of serializing two string arrays. A project making extensive use of the chart system
 * could end up storing thousands of markers (and thus instances of metadata).
 *
 ...
 * hierarchies. It's also much easier to write tools to automate addition, removal, aggregation and visualization
 * if everything is based on strings.*/
